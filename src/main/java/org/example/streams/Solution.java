...
class RunCollection {
  ...
  public int personalBest() {
    // Returns the best time achieved in this RunCollection
    //check if the run is complete, if completed return the best from the completed
    //int best = Integer.MAX_VALUE;
   // for(int i = 0; i < runs.size(); i++){
    //  Run run = runs.get(i);
      //System.out.print(run.complete);
      //System.out.print(run.obstacleTimes);
  ...
  public int bestOfBests() {
    ...
    for(int i = 0; i < runs.size(); i++){
      ...
      //4 runs
      //compare each runtimne from run for each obstacle
      //whichever is the lowest , add it tth best time
      Run run = runs.get(i);
  //    run.obstacleTimes()
     // run.get
      //System.out.print(run.complete);
